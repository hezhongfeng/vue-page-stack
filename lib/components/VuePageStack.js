...
const VuePageStack = () => {
  return defineComponent({
    ...
    setup(_props, { slots, emit }) {
      ...
      let useCache = false;
      const cacheSubtree = () => {
        console.log('cacheSubtree start', history.action, pendingCacheKey);
        if (pendingCacheKey) {
          console.log(106);
          if (useCache) {
        ...
        console.log('cacheSubtree end', stack);
      };
      ...
      return () => {
        ...
        if (history.action === config.backName) {
          console.log('走 back 逻辑');
          emit('back');
          console.log('走 back 缓存逻辑');
