...
const hasMatchingCacheKey = (cachedVNode, currentVNode) =>
  cachedVNode && cachedVNode.key != null && currentVNode.key != null && cachedVNode.key === currentVNode.key;
...
const restoreFromCache = ({ stack, step, currentVNode, vnode, reuseCachedVNode, resetShapeFlag }) => {
  const cachedVNode = stack[stack.length - step - 1];

  if (!hasMatchingCacheKey(cachedVNode, currentVNode)) {
...
const createPageStackState = ({ getInnerChild, resetShapeFlag, reuseCachedVNode, unmountCached }) => {
  ...
  return {
    ...
    restore(currentVNode, vnode, step) {
...
const handleBackNavigation = ({ emit, navigationState, vnode, pageStackState, renderState }) => {
  emit(STACK_EVENTS.back);

  const restored = pageStackState.restore(vnode, vnode, -navigationState.n);
  if (!restored) {
    return false;
  }
  ...
  setup(_props, { slots, emit }) {
    ...
    return () => {
      ...
      if (navigationState.action === NAVIGATION_ACTIONS.back) {
        if (!handleBackNavigation({ emit, navigationState, vnode, pageStackState, renderState })) {
